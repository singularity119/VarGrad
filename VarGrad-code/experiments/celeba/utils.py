...
delta_stats = [f"attr_{idx}" for idx in range(40)]
BASE = np.array(
    [
        0.6736886,
        0.68121034,
    ...
    dtype=np.float32,
)  # single-task F1 baseline from the FAMO CelebA issue
SIGN = np.ones(40)
KK = np.ones(40) * -1
...
def delta_fn(a):
    ...
    if a.shape[-1] != BASE.shape[0]:
        raise ValueError(f"CelebA delta_m expects 40 F1 values, got shape {a.shape}.")
